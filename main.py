...
"""本文件作为工程服务入口文件，需要固化在镜像中，作为启动入口"""
######################自动添加自定义模块######################
def append_water_module(BASE_DIR):
    fullFile = find_file(BASE_DIR,'initProcessors.py')
    os.system(f"python {fullFile} {BASE_DIR}")
...
if __name__ == '__main__':
    ...
    BASE_DIR = sys.argv[1]
    """编译国际化文件"""
    os.system(f"cd {BASE_DIR} && python manage.py compilemessages")

    os.environ['HF_HOME'] = '/opt/water/model/base'
