...
class ConfigManager:
    ...
    def load_from_env(self):
        ...
        config = {key.replace('WATER_', ''): os.environ.get(key) for key in keys if key.startswith('WATER_')}
        if len(config.keys()) <= 0:
            msg = f"""
                             ...
                             Please set environment variables
                                WATER_CONFIG_TYPE: 配置文件读取方式 FILE: 使用配置文件配置  ENV: 使用ENV配置
                                WATER_DB_NAME: 数据库名称
                                WATER_DB_HOST: 数据库主机
                                WATER_DB_PORT: 数据库端口
                                WATER_DB_USER: 数据库用户名
                                WATER_DB_PASSWORD: 数据库密码
                                WATER_EMBEDDING_MODEL_PATH: 向量模型目录
                                WATER_EMBEDDING_MODEL_NAME: 向量模型名称
                             """
            raise ImportError(msg)
    ...
    def load_user_config(cls, root_path=None, config_class=None):
        ...
        config_type = os.environ.get('WATER_CONFIG_TYPE')
